...
class NexusAgent(BaseAgent):
    ...
    def prepareAndPublish(self, nexIDs, tracking):
        ...
        mavenmetafile = urllib2.urlopen(self.config.get("baseUrl", '')+"repository/"+repoid+"/"+groupid+"/"+nexIDs["name"]+"/maven-metadata.xml")#reading base mavenmetadata file to fetch main version
        mavenmetadata = xmltodict.parse(mavenmetafile.read())
        mavenmetafile.close()
        lastupdated = mavenmetadata["metadata"]["versioning"]["lastUpdated"]
        if tracking>0:
    ...
    def publishdata(self, repoid, groupid, nexIDs, version, artifactid, lastupdated):
        data = []
        mainmavenxml = urllib2.urlopen(self.config.get("baseUrl", '')+"repository/"+repoid+"/"+groupid+"/"+nexIDs["name"]+"/"+version+"/"+nexIDs["name"]+"-"+version+".pom")#reading mavenmetadata file inside main version folder
